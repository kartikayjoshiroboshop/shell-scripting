...
## Use cases
# 1. If we want to use a value in multiple places then we go with variables. This brings and advantage of changing the value in one place and it impacts in all the places.
...
## 2. Usecase: If we need to get the data dynamically we use variables to store that data and we refer
  ...
  #date --help gives you multiple options
  DATE=$(date +%F)
